...
def buat_laporan():
    ...
    hasil_laboratorium = [
        ...
        ["Total Kolestrol Darah: ", str(st.session_state.total_kolestrol_darah)+" mg/dL", " <220 mg/dL"],
    ]
...
if st.session_state.lanjut_pemeriksaan == 1:
    ...
    with st.form(key="form_faktor_bisa_diubah"):
        ...
        if st.form_submit_button("Lanjut"):
            ...
            if st.session_state.total_kolestrol_darah > 220.0 or st.session_state.total_kolestrol > 220.0 or (st.session_state.HDL != 0 and st.session_state.HDL < 30.0 or st.session_state.LDL >= 130.0 or st.session_state.trigliserida >= 200.0):
                st.session_state.faktor_risiko_2 = st.session_state.faktor_risiko_2 + 1
                
                disiplidemia = "Disiplidemia: "
                if st.session_state.total_kolestrol_darah > 220:
            ...
            #Jika mengalami disiplidemia dengan HDL <= 35 dan/atau LDL >= 100 dan/atau trigliserida ≥  200.0
                ...
                if st.session_state.trigliserida >= 200.0:
